...
export const getWaveCommentsById = async (req: any, res: any) => {
  try {
    ...
    const waveComments = await Comments.findAll({
      where: { waveId: id }, // Sequelize query to find comments where userId matches
      include: [
        {
          model: Users, // Include the Users model for user details
          as: "userComment", // Alias used in the association
          // attributes: ["id", "name", "email"], // Specify the user fields to include
        },
      ],
    });
