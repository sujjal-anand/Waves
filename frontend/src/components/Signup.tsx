    ...
    try {
      ...
      if (response.status === 201) {
        ...
        if (token) {
          ...
          await api.post(`${Local.ADD_FRIEND_SIGNUP}`,{receiverfriendId:response?.data?.user?.id}, createAuthHeaders(token));
        }
      }
    } catch (error: any) {
    ...
    <div className="container-fluid">
      <div className="row" style={{ height: '60%' }}>
        ...
        <div className="d-flex col-6 p-0">
          <img src={image} className="img-fluid" style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="Signup" />
        </div>

        {/* Form Column */}
        <div className="col-6 bg-white p-3">
          <div style={{ marginTop: '50px', marginLeft: '113px' }}>
            <h2 className="row-12 mb-4" style={{ marginLeft: '21px' }}>Sign Up</h2>
            <Formik
              initialValues={{
                firstName: '',
                lastName: '',
            ...
            >
              {() => (
                <Form className="row g-3 p-4">
                  ...
                  {/* Submit Button */}
                  <div className="col-12">
                    <button type="submit" className="btn btn-primary col-4 p-2">Sign Up</button>
                  </div>
                </Form>
              )}
            </Formik>
