...
const getLatestWaves = async () => {
  ...
  const token= localStorage.getItem("token")
  if(token){
      const response = await api.get(`${Local.LATEST_WAVES}`, createAuthHeaders(token));
      if (response.status !== 200) {
      throw new Error('Failed to fetch user details');
...
const getAcceptedFriends = async () => {
  ...
  const token= localStorage.getItem("token")
  if(token){
      const response = await api.get(`${Local.GET_ACCEPTED_FRIENDS}`, createAuthHeaders(token));
      if (response.status !== 200) {
      throw new Error('Failed to fetch user details');
...
const Dashboard = () => {
  ...
  const { data:wavesDetail, error, isLoading, isError } = useQuery({
...
console.log(acceptedFriends)
  ...
  <div
    ...
    onClick={() => {
      // setGetfriend(friend.friend_1); or setGetfriend(friend.friend_2);
    }}
  >
