...
const Dashboard = () => {
  ...
  const handleSaveClick = async (commentId: number) => {
    try {
      ...
      if (response.status === 200) {
        queryClient.invalidateQueries({queryKey:["waveComments"]})
  ...
  const { data: waveComments } = useQuery({
    queryKey: ["waveComments",id],
    queryFn: getWaveComments,
  });
        ...
        >
          <div className="modal-dialog modal-dialog-centered modal-lg ">
            <div className="modal-content">
              ...
              >
                {waveComments?.data?.map((comment: any) => (
             <div
                    key={comment.id}
