...
const fetchFriendsList=async()=>{
  ...
  if(token){
    const response = await api.get(`${Local.GET_FRIENDS_LIST}`, createAuthHeaders(token));
              ...
              >
                <div className="d-flex align-items-center">
                  ...
                  <div className="pt-1 ps-2">
                    <img
                      src={`https://api.dicebear.com/5.x/initials/svg?seed=${friends?.friend?.User?.firstName} ${friends?.friend?.User?.lastName}`}
                      alt="Profile picture"
                  ...
                  <div className="ms-3 small">
                    <p
                      ...
                      style={{ color: "#3E5677", fontSize: "16px" }} // Increased font size
                    >
                      {friends?.friend?.User?.email}
                    </p>
                    <p
                      className="mt-0"
                      style={{ fontSize: "15px", color: "#555" }} // Increased font size
                    >
                      {`${friends?.friend?.User?.firstName} ${friends?.friend?.User?.lastName}`}
                    </p>
                  </div>
